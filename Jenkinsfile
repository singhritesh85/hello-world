    ...
    stages{
        ...
        stage("Build"){
            steps{
                ...
                sh 'scp -rv webapp docker-user@3.144.82.25:~/'
                sh 'scp -rv Dockerfile docker-user@3.144.82.25:~/'
                sh 'scp -rv docker-compose.yaml docker-user@3.144.82.25:~/'
            }
        }
        stage("Deployment"){
            steps{
                sh '''
                       ssh docker-user@3.144.82.25 << -EOT
                       docker-compose -p newproject build --no-cache
                       docker tag 027330342406.dkr.ecr.us-east-2.amazonaws.com/newecr:latest-prod
